...
//
// Modified by kawakatz on 2025-09-13:
// - Exported multiple previously-internal identifiers to allow reuse from external packages
//   (e.g., CallbackPortUsage).
// - Made Start function runnable without a Docker environment by removing container-specific assumptions
//
// Upstream reference (freeze this to a commit permalink):
// https://github.com/its-a-feature/Mythic/blob/master/mythic-docker/src/rabbitmq/utils_proxy_traffic.go
...
func (p *CallbackPortUsage) Start() error {
	...
	case CALLBACK_PORT_TYPE_SOCKS:
		...
		//go SendAllOperationsMessage(fmt.Sprintf("Opened port %d for %s", p.LocalPort, p.PortType), p.OperationID, "", database.MESSAGE_LEVEL_INFO)
	case CALLBACK_PORT_TYPE_RPORTFWD:
	...
	case CALLBACK_PORT_TYPE_INTERACTIVE:
		if isPortExposedThroughDocker(p.LocalPort) {
			...
			go p.manageConnections()
			//go SendAllOperationsMessage(fmt.Sprintf("Opened port %d for %s", p.LocalPort, "interactive tasking"), p.OperationID, "", database.MESSAGE_LEVEL_INFO)
			...
			case CALLBACK_PORT_TYPE_RPORTFWD:
				...
				if _, ok := connectionMap[newMsg.ServerID]; ok {
					...
					if err != nil {
						...
						//go SendAllOperationsMessage(fmt.Sprintf("Failed to connect to %s:%d for new rpfwd message", p.RemoteIP, p.RemotePort), p.OperationID, "rpfwd", database.MESSAGE_LEVEL_WARNING)
					} else {
			...
			go func(conn net.Conn) {
				...
				for {
					...
					if length > 0 {
						...
						fmt.Printf("Message received: server_id -> %d, length -> %d\n", newConnection.ServerID, length)
						/*interceptProxyToAgentMessageChan <- interceptProxyToAgentMessage{
							Message: ProxyToAgentMessage{
								Message:  buf[:length],
								IsExit:   err != nil,
